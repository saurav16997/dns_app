...
def fibonacci_num():
    try:
        ...
        msg_from_fs = str(fs_dict)
        bytes_sent = str.encode(msg_from_fs)
        print(msg_from_fs)
        buffer = 1024
        as_server_address = (as_ip, as_port)
        fs_udp_socket = socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)
        fs_udp_socket.sendto(bytes_sent, as_server_address)
