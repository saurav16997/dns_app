...
def get_fs_body(as_ip, as_port):
    ...
    msg_from_as = "FS Server has been registered to Authoritative Server"
...
def dns_lookup(fs_dict, us_dict):
    fs_dict = str(fs_dict, encoding='utf-8')
    us_dict = str(us_dict, encoding='utf-8')
    us_dict = ast.literal_eval(us_dict)
...
FS_Dict = get_fs_body(AS_IP, AS_Port)
US_Values = dns_us_request_response(AS_IP, AS_Port)
US_Dict = US_Values[0]
